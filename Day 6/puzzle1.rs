...
fn main() -> io::Result<()> {
    ...
    for i in 1..(information[0].len()){
        ...
        let game = Game {
            duration: information[0][i].parse::<f32>().unwrap(),
            distance: information[1][i].parse::<f32>().unwrap(),
        };
...
fn get_lower_bound(time: f32, distance: f32) -> i32 {

    let radical = f32::sqrt(time * time - 4.0 * distance);
...
fn get_upper_bound(time: f32, distance: f32) -> i32 {
